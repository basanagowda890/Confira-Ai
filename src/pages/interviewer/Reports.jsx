...
export default function Reports() {
  ...
  const candidateName = interview?.candidate?.full_name || interview?.profiles?.full_name || (interview?.title?.includes("—") ? interview.title.split("—")[1]?.trim() : "") || "Candidate";
  return <div><SectionTitle eyebrow="REPORTS" title={`${candidateName} — Interview report`} description={interview ? `${candidateName} · ${interview.title}` : "Reports generated from completed interviews."} action={<div className="btn-group"><button className="btn btn-outline" onClick={() => navigator.clipboard?.writeText(window.location.href)}><Share2 size={15}/> Share</button><button className="btn btn-primary" onClick={() => window.print()}><Download size={15}/> Download report</button></div>} />
    <div className="candidate-picker card"><div><h3>View candidate report</h3><p>Review reports generated from shared interview records.</p></div><select value={selectedId} onChange={event => selectInterview(event.target.value)}>{interviews.map(item => <option value={item.id} key={item.id}>{(item.candidate?.full_name || item.profiles?.full_name || (item.title?.includes("—") ? item.title.split("—")[1]?.trim() : "") || "Candidate")} · {item.title}</option>)}</select></div>
