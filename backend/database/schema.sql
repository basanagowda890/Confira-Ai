...
alter table public.interview_answers add column if not exists ai_assistance_score numeric check(ai_assistance_score between 0 and 100);
alter table public.interview_answers add column if not exists ai_assistance_classification text check(ai_assistance_classification in ('low','medium','high'));
alter table public.interview_answers add column if not exists ai_assistance_confidence text check(ai_assistance_confidence in ('low','medium','high'));
alter table public.interview_answers add column if not exists ai_assistance_signals jsonb not null default '[]';
alter table public.interview_answers add column if not exists ai_assistance_explanation text;
...
create table if not exists public.group_discussions (id uuid primary key default gen_random_uuid(), job_id uuid references jobs(id), created_by uuid not null references profiles(id), title text not null, topic text, scheduled_at timestamptz, started_at timestamptz, ended_at timestamptz, duration_minutes integer not null default 30, current_speaker_id uuid references profiles(id), status text not null default 'scheduled' check(status in ('scheduled','live','paused','completed','cancelled')), meeting_room_id text, created_at timestamptz not null default now());
alter table public.group_discussions add column if not exists topic text;
alter table public.group_discussions add column if not exists duration_minutes integer default 30;
alter table public.group_discussions add column if not exists started_at timestamptz;
alter table public.group_discussions add column if not exists ended_at timestamptz;
alter table public.group_discussions add column if not exists meeting_room_id text;
alter table public.group_discussions add column if not exists current_speaker_id uuid references profiles(id);

create table if not exists public.group_discussion_members (discussion_id uuid references group_discussions(id) on delete cascade, candidate_id uuid references profiles(id), joined_at timestamptz default now(), left_at timestamptz, status text default 'invited' check(status in ('invited','joined','speaking','left')), primary key(discussion_id,candidate_id));
alter table public.group_discussion_members add column if not exists left_at timestamptz;
alter table public.group_discussion_members add column if not exists status text default 'invited';

create table if not exists public.group_discussion_messages (id uuid primary key default gen_random_uuid(), discussion_id uuid references group_discussions(id) on delete cascade, sender_id uuid references profiles(id), sender_name text, message text not null, message_type text default 'text', created_at timestamptz not null default now());

create table if not exists public.group_discussion_transcripts (id uuid primary key default gen_random_uuid(), discussion_id uuid references group_discussions(id) on delete cascade, candidate_id uuid references profiles(id), speaker_name text, text text not null, duration_seconds integer default 0, timestamp timestamptz not null default now());
...
create index if not exists gd_messages_idx on group_discussion_messages(discussion_id,created_at);
create index if not exists gd_transcripts_idx on group_discussion_transcripts(discussion_id,candidate_id,timestamp);
